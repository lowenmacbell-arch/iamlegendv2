 ...
 *                                                                           *
 *    Description: Unit tests for message handlers (ᴵ ᴬᴹ ᴸᴱᴳᴱᴺᴰ Bot)        *
 *                                                                           *
 ***************************************************************************/
...
vi.mock('../../config.js', () => ({ default: { ownerNumber: '255618558502@s.whatsapp.net', prefixes: ['.'], botName: 'ᴵ ᴬᴹ ᴸᴱᴳᴱᴺᴰ', botMode: 'public' } }));
...
describe('handleMessages', () => {
    ...
    it('sends error response when command handler throws', async () => {
        ...
        const msg = createMockMessage({ text: '.crashtest', isGroup: true, senderId: '255618558502@s.whatsapp.net' });
        await handleMessages(sock, { messages: [msg], type: 'notify' });
...
describe('handleGroupParticipantUpdate', () => {
    ...
    it('handles add without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363000000000001@g.us', participants: ['255618111111@s.whatsapp.net'],
            action: 'add', author: '255619999999@s.whatsapp.net',
    ...
    it('handles remove without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363000000000001@g.us', participants: ['255618111111@s.whatsapp.net'],
            action: 'remove', author: '255619999999@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
    ...
    it('handles promote without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363000000000001@g.us', participants: ['255618111111@s.whatsapp.net'],
            action: 'promote', author: '255619999999@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
    ...
    it('handles demote without crashing', async () => {
        await expect(handleGroupParticipantUpdate(sock, {
            id: '120363000000000001@g.us', participants: ['255618111111@s.whatsapp.net'],
            action: 'demote', author: '255619999999@s.whatsapp.net',
        })).resolves.not.toThrow();
    });
...
describe('handleCall', () => {
    ...
    it('warns caller when anticall enabled', async () => {
        await handleCall(sock, [{ id: 'call-1', from: '255618111111@s.whatsapp.net' }]);
        expect(sock.sendMessage).toHaveBeenCalledWith(
            '255618111111@s.whatsapp.net',
    ...
    it('rejects call via rejectCall', async () => {
        await handleCall(sock, [{ id: 'call-1', from: '255618111111@s.whatsapp.net' }]);
        expect(sock.rejectCall).toHaveBeenCalledWith('call-1', '255618111111@s.whatsapp.net');
    });
    ...
    it('handles call without id gracefully', async () => {
        await expect(handleCall(sock, [{ from: '255618111111@s.whatsapp.net' }])).resolves.not.toThrow();
    });
});